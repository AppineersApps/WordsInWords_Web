 ...
 * @module Users
 *
 * @class Users_guest_model.php
 *
 * @path application\webservice\basic_appineers_master\models\Users_guest_model.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Users_guest_model extends CI_Model
{
    ...
    /**
     * create_guest_user method is used to execute database queries for User Sign Up Email API.
     * @created priyanka chillakuru | 12.09.2019
     * @modified priyanka chillakuru | 23.12.2019
     * @param array $params_arr params_arr array to process query block.
     * @return array $return_arr returns response of query block.
     */
    public function create_guest_user($params_arr = array())
    {
        try {
            $result_arr = array();
            ...
            if (isset($params_arr["guest_user_id"])) {
                $this->db->set("iGuestUserId", $params_arr["guest_user_id"]);
            }
            if (isset($params_arr["guest_user"])) {
                $this->db->set("eGuestUser", $params_arr["guest_user"]);
            }
    ...
    /**
     * user_guest_details method is used to execute database queries for User Sign Up Email API.
     * @created priyanka chillakuru | 12.09.2019
     * @modified priyanka chillakuru | 01.10.2019
     * @param string $insert_id insert_id is used to process query block.
     * @return array $return_arr returns response of query block.
     */
    public function user_guest_details($insert_id = '')
    {
        try {
            $result_arr = array();
    ...
    /**
     * get_user_details_v2 method is used to execute database queries for User Sign Up Phone API.
     * @created CIT Dev Team
     * @modified priyanka chillakuru | 23.12.2019
     * @param string $insert_id insert_id is used to process query block.
     * @return array $return_arr returns response of query block.
     */
    public function get_user_details_v2($insert_id = '')
    {
        try {
            $result_arr = array();
            ...
            $this->db->select("u.dtAddedAt AS u_added_at");
            $this->db->select("u.dtUpdatedAt AS u_updated_at");
            $this->db->select("u.vAccessToken AS auth_token1");
            $this->db->select("u.eSocialLoginType AS u_social_login_type");
            $this->db->select("u.vSocialLoginId AS u_social_login_id");
            $this->db->select("u.ePushNotify AS u_push_notify");
            //$this->db->select("ms.vState AS ms_state");
    ...
    public function connect_guest_user($params_arr = array(), $where_arr = array())
    {
     ...
     /**
     * social_account_connect_guest_user method is used to execute database queries for User Sign Up Email API.
     * @created priyanka chillakuru | 12.09.2019
     * @modified priyanka chillakuru | 23.12.2019
     * @param array $params_arr params_arr array to process query block.
     * @return array $return_arr returns response of query block.
     */
    public function social_account_connect_guest_user($params_arr = array(), $where_arr = array())
    {
        try {
            $result_arr = array();
            if (!is_array($params_arr) || count($params_arr) == 0) {
