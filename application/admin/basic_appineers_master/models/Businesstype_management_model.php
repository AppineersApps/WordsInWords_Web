...
/**
 * Description of businesstype Management Model
 *
 * @category admin
 *
 ...
 *
 * @class businesstype_management_model.php
 *
 * @path application\admin\basic_appineers_master\models\Businesstype_management_model.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Businesstype_management_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->primary_alias = "b_business_type_id";
        $this->physical_data_remove = "Yes";
        $this->grid_fields = array("b_name",
                                    "b_status"
                                );
    ...
    public function getData($extra_cond = "", $fields = "", $order_by = "", $group_by = "", $limit = "", $join = "Yes", $having_cond = '', $list = FALSE)
    {
        if(is_array($fields)){
            ...
            $this->db->select("b.vName AS b_name");
            $this->db->select("b.eStatus AS b_status");

        } else {

            $this->db->select("b.iBusinessTypeId AS b_bussiness_type_id");
            $this->db->select("b.vName AS b_name");
            $this->db->select("b.eStatus AS b_status");
            $this->db->select("b.dtAddedAt AS b_added_at");
            $this->db->select("b.dtUpdatedAt AS b_updated_at");
            $this->db->select("b.dtDeletedAt AS b_deleted_at");
        }
    ...
    public function getListConfiguration($name = "")
    {
        $list_config = array(
            ...
            "b_status" => array(
                ...
                "table_alias" => "b",
                "field_name" => "eStatus",
                "source_field" => "b_status",
                "display_query" => "b.eStatus",
                "entry_type" => "Table",
                "data_type" => "varchar",
                "show_in" => "Both",
                "type" => "textbox",
    ...
    public function getFormConfiguration($name = "")
    {
        $form_config = array(
            ...
            "type" => "date",
            "label" => "Updated At",
            "lang_code" => "BUSINESSTYPE_MANAGEMENT_UPDATED_AT",
            "label_lang" => $this->lang->line('BUSINESSTYPE_MANAGEMENT_UPDATED_AT'),
            "default" => $this->filter->getDefaultValue("b_updated_at","MySQL","NOW()"),
            "dfapply" => "forceApply",
            "format" => $this->general->getAdminPHPFormats('date')
        ),
                ...
                "type" => "date",
                "label" => "Deleted At",
                "lang_code" => "BUSINESSTYPE_MANAGEMENT_DELETED_AT",
                "label_lang" => $this->lang->line('BUSINESSTYPE_MANAGEMENT_DELETED_AT'),
                "format" => $this->general->getAdminPHPFormats('date')
            ),
                "b_status" => array(
                "name" => "b_status",
