...
/**
 * Description of Businesstype Management Controller
 *
 * @category admin
 *
 ...
 *
 * @module Businesstype Management
 *
...
class Businesstype_management extends Cit_Controller
{
        ...
        try
        {
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                $access_list = array(
                    ...
                    "businesstype_management_export",
                    "businesstype_management_print",
                );
    ...
    public function export()
    {
        ...
        $table_alias = $this->businesstype_management_model->table_alias;
        ...
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("businesstype_management", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            $b_name = $params_arr["b_name"];
            $b_deleted_at = $params_arr["b_deleted_at"];
            $b_added_at = $params_arr["b_added_at"];
            $b_updated_at = $params_arr["b_updated_at"];
            $b_status = $params_arr["b_status"];
          

            $data = $save_data_arr = $file_data = array();
            $data["vName"] = $b_name;
            $data["dtAddedAt"] = $this->filter->formatActionData($b_added_at, $form_config["b_added_at"]);
            $data["dtUpdatedAt"] = $this->filter->formatActionData($b_updated_at, $form_config["b_updated_at"]);
            $data["dtDeletedAt"] = $this->filter->formatActionData($b_deleted_at, $form_config["b_deleted_at"]);
            $data["eStatus"] = $b_status;


            $save_data_arr["b_name"] = $data["vName"];
            $save_data_arr["b_deleted_at"] = $data["dtDeletedAt"];
            $save_data_arr["b_status"] = $data["eStatus"];
            $save_data_arr["b_added_at"] = $data["dtAddedAt"];
            $save_data_arr["b_updated_at"] = $data["dtUpdatedAt"];
            $strCheck = $this->checkUniqueBussinessType($id);
            if ($mode == 'Add') 
            {
                ...
                $track_cond = $this->db->protect("b.iBusinessTypeId")." = ".$this->db->escape($id);
            ...
            elseif ($mode == 'Update' && true==$strCheck && 'Active' == $data["eStatus"])
            {
            ...
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            if(true==$strCheck && 'Inactive' == $data["eStatus"])
            {
                $ret_arr['message'] = "This bussiness type is already in used. So can not be Deleted or Inactivated";
            }else{
                $ret_arr['message'] = $msg; 
            }
    ...
    public function inlineEditAction()
    {
        ...
        $strCheck = $this->checkUniqueBussinessType($primary_ids);
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if (is_array($primary_ids))
                    {
                        $main_cond = $this->db->protect("b.iBusinessTypeId")." IN ('".implode("','", $primary_ids)."')";
                    }
                    else
                    {
                        $main_cond = $this->db->protect("b.iBusinessTypeId")." = ".$this->db->escape($primary_ids);
                    }
                    if(false==$strCheck)
                    {
                ...
                case 'edit':
                    ...
                    if (method_exists($this, 'ActiveBusinesstypeInlineEdition'))
                    {
                        $event_res = $this->ActiveBusinesstypeInlineEdition($field_name, $post_val, $primary_ids, $params_arr['parID'], $params_arr['parMod']);
                ...
                case 'status':
                    ...
                    if (method_exists($this, 'ActiveBusinesstypeAfterChangeStatus'))
                    {
                        $event_res = $this->ActiveBusinesstypeAfterChangeStatus($params_arr['status'], $primary_ids, $params_arr['parID'], $params_arr['parMod']);
                        $success = $event_res['success'];
                        if (!$event_res['success'])
                        {
                            $edit_error_msg = $this->general->processMessageLabel('ACTION_AFTER_EVENT_HAS_BEEN_FAILED_C46_C46_C33');
            ...
            if(true==$strCheck){
                $message = "This bussiness type is already in used. So can not be Deleted or Inactivated";
            }
