...
/**
 * Description of Review Management Controller
 *
 * @category admin
 *
 ...
 *
 * @class Review_management.php
 *
 * @path application\admin\basic_appineers_master\controllers\Review_management.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Review_management extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('review_management_model');
        ...
        $this->dropdown_arr = array(
            "r_status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('REVIEW_MANAGEMENT_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('REVIEW_MANAGEMENT_INACTIVE')
                    )
                )
            )
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "review_management_list",
                    "review_management_view",
                    "review_management_add",
                    "review_management_update",
                    "review_management_delete",
                    "review_management_export",
                    "review_management_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("review_management", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "review_management");
            ...
            $status_label = array(
                'js_lang_label.REVIEW_MANAGEMENT_ACTIVE',
                'js_lang_label.REVIEW_MANAGEMENT_INACTIVE'
            );

            $list_config = $this->review_management_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "review_management");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->review_management_model->default_filters,
            );
            ...
            {
                $this->loadView("review_management_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->review_management_model->getListConfiguration();
        $form_config = $this->review_management_model->getFormConfiguration();
        $extra_cond = $this->review_management_model->extra_cond;
        $groupby_cond = $this->review_management_model->groupby_cond;
        $having_cond = $this->review_management_model->having_cond;
        $orderby_cond = $this->review_management_model->orderby_cond;
        ...
        $data_recs = $this->review_management_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_REVIEW_MANAGEMENT_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("review_management_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("review_management", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->review_management_model->getListConfiguration();
        $form_config = $this->review_management_model->getFormConfiguration();
        $table_name = $this->review_management_model->table_name;
        $table_alias = $this->review_management_model->table_alias;
        $primary_key = $this->review_management_model->primary_key;
        $extra_cond = $this->review_management_model->extra_cond;
        $groupby_cond = $this->review_management_model->groupby_cond;
        $having_cond = $this->review_management_model->having_cond;
        $orderby_cond = $this->review_management_model->orderby_cond;
        ...
        $db_recs = $this->review_management_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('REVIEW_MANAGEMENT_REVIEW_MANAGEMENT');
        $misc_info['filename'] = "review_management_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("review_management_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("review_management", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("review_management", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "review_management"), $this->session->userdata('iAdminId'));
                $data_arr = $this->review_management_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "review_management", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "review_management"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "review_management");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->review_management_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->review_management_model, $phpfunc))
                            {
                                $tmpdata = $this->review_management_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->review_management_model, $fd_callback))
                                {
                                    $fd_status = $this->review_management_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("review_management_add");
                    }
                    else
                    {
                        $this->loadView("review_management_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("review_management_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("review_management_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("review_management", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $r_star = $params_arr["r_star"];
            $r_description = $params_arr["r_description"];
            $r_profile_image = $params_arr["r_profile_image"];
            $r_first_name = $params_arr["r_first_name"];
            $r_last_name = $params_arr["r_last_name"];
            $r_email = $params_arr["r_email"];
            $r_mobile_no = $params_arr["r_mobile_no"];
            $r_address = $params_arr["r_address"];
            $r_city = $params_arr["r_city"];
            $r_state_id = $params_arr["r_state_id"];
            $r_zip_code = $params_arr["r_zip_code"];
            $r_deleted_at = $params_arr["r_deleted_at"];
            $r_added_at = $params_arr["r_added_at"];
            $r_updated_at = $params_arr["r_updated_at"];
            $r_status = $params_arr["r_status"];
            $r_latitude = $params_arr["r_latitude"];
            $r_longitude = $params_arr["r_longitude"];
            $r_review_type = $params_arr["r_review_type"];
            $r_user_id = $params_arr["r_user_id"];
            $r_business_name = $params_arr["r_business_name"];
            $r_business_type = $params_arr["r_business_type"];
            $r_position = $params_arr["r_position"];
            $r_place_id = $params_arr["r_place_id"];
            $r_claimed_email = $params_arr["r_claimed_email"];
            $r_is_claimed = $params_arr["r_is_claimed"];
          

            $data = $save_data_arr = $file_data = array();
            $data["vProfileImage"] = $r_profile_image;
            $data["vFirstName"] = $r_first_name;
            $data["vLastName"] = $r_last_name;
            $data["vEmail"] = $r_email;
            $data["vMobileNo"] = $r_mobile_no;
            $data["tAddress"] = $r_address;
            $data["vCity"] = $r_city;
            $data["iStateId"] = $r_state_id;
            $data["vZipCode"] = $r_zip_code;
            $data["dAddedAt"] = $this->filter->formatActionData($r_added_at, $form_config["r_added_at"]);
            $data["dtUpdatedAt"] = $this->filter->formatActionData($r_updated_at, $form_config["r_updated_at"]);
            $data["dtDeletedAt"] = $this->filter->formatActionData($r_deleted_at, $form_config["r_deleted_at"]);
            $data["eStatus"] = $r_status;
            $data["dLatitude"] = $r_latitude;
            $data["dLongitude"] = $r_longitude;
            $data["iStars"] = $r_star;
            $data["vDescription"] = $r_description;
            $data["iUserId"] = $r_user_id;
            $data["vBussinessName"] = $r_business_name;
            $data["iBussinessType"] = $r_business_type;
            $data["vPosition"] = $r_position;
            $data["vPlaceId"] = $r_place_id;
            $data["vClaimedEmail"] = $r_claimed_email;
            $data["bIsClaimed"] = $r_is_claimed;


            $save_data_arr["r_profile_image"] = $data["vProfileImage"];
            $save_data_arr["r_first_name"] = $data["vFirstName"];
            $save_data_arr["r_last_name"] = $data["vLastName"];
            $save_data_arr["r_email"] = $data["vEmail"];
            $save_data_arr["r_mobile_no"] = $data["vMobileNo"];
            $save_data_arr["r_address"] = $data["tAddress"];
            $save_data_arr["r_city"] = $data["vCity"];
            $save_data_arr["r_state_id"] = $data["iStateId"];
            $save_data_arr["r_zip_code"] = $data["vZipCode"];
            $save_data_arr["r_deleted_at"] = $data["dtDeletedAt"];
            $save_data_arr["r_status"] = $data["eStatus"];
            $save_data_arr["r_latitude"] = $data["dLatitude"];
            $save_data_arr["r_longitude"] = $data["dLongitude"];
            $save_data_arr["r_added_at"] = $data["dtAddedAt"];
            $save_data_arr["r_updated_at"] = $data["dtUpdatedAt"];
            $save_data_arr["r_star"] = $data["iStars"];
            $save_data_arr["r_description"] = $data["vDescription"];
            $save_data_arr["r_user_id"] = $data["iUserId"];
            $save_data_arr["r_business_name"] = $data["vBussinessName"];
            $save_data_arr["r_business_type"] = $data["vBusinessType"];
            $save_data_arr["r_position"] = $data["vPosition"];
            $save_data_arr["r_place_id"] = $data["vPlaceId"];
            $save_data_arr["r_claimed_email"] = $data["vClaimedEmail"];
            $save_data_arr["r_is_claimed"] = $data["bIsClaimed"];
            if ($mode == 'Add')
            {
                $id = $this->review_management_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iReviewId"] = $data["iReviewId"] = $id;
                ...
                $track_cond = $this->db->protect("r.iReviewId")." = ".$this->db->escape($id);
                $switch_combo = $this->review_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "review_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->review_management_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iReviewId"] = $data["iReviewId"] = $id;
                ...
                $switch_combo = $this->review_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"],"review_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
            ...
            $file_data["r_profile_image"]["file_name"] = $r_profile_image;
            $file_data["r_profile_image"]["old_file_name"] = $params_arr["old_r_profile_image"];
            $file_data["r_profile_image"]["unique_name"] = "r_profile_image";
            $file_data["r_profile_image"]["primary_key"] = "iReviewId";
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->review_management_model->getListConfiguration();
            $config_arr['form_config'] = $this->review_management_model->getFormConfiguration();
            $config_arr['table_name'] = $this->review_management_model->table_name;
            $config_arr['table_alias'] = $this->review_management_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->review_management_model->table_alias.".".$this->review_management_model->primary_key;
        }
        else
        {
            $primary_field = $this->review_management_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("review_management_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("review_management", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    if (is_array($primary_ids))
                    {
                        $main_cond = $this->db->protect("r.iReviewId")." IN ('".implode("','", $primary_ids)."')";
                    }
                    else
                    {
                        $main_cond = $this->db->protect("r.iReviewId")." = ".$this->db->escape($primary_ids);
                    }
                    $main_data = $this->review_management_model->getData($main_cond, array('r.vProfileImage'));
                    $success = $this->review_management_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("review_management_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("review_management", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->review_management_model->getListConfiguration($post_name);
                    $form_config = $this->review_management_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $db_rec_arr = $this->review_management_model->getData(intval($primary_ids));

                    $data_arr[$field_name] = $post_val;
                    $success = $this->review_management_model->update($data_arr, intval($primary_ids));
                    $message = $this->general->processMessageLabel('ACTION_RECORD_SUCCESSFULLY_UPDATED_C46_C46_C33');
                    if (method_exists($this, 'ActiveReviewInlineEdition'))
                    {
                        $event_res = $this->ActiveReviewInlineEdition($field_name, $post_val, $primary_ids, $params_arr['parID'], $params_arr['parMod']);
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->review_management_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    $success = $this->review_management_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                    ...
                    if (method_exists($this, 'ActiveReviewAfterChangeStatus'))
                    {
                        $event_res = $this->ActiveReviewAfterChangeStatus($params_arr['status'], $primary_ids, $params_arr['parID'], $params_arr['parMod']);
                        $success = $event_res['success'];
                        if (!$event_res['success'])
                        {
                            $edit_error_msg = $this->general->processMessageLabel('ACTION_AFTER_EVENT_HAS_BEEN_FAILED_C46_C46_C33');
            ...
            if ($mode == "Update")
            {
                ...
                if ($form_config["type"] == "file")
                {
                    ...
                    $file_data[$unique_name]["primary_key"] = "iReviewId";
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->review_management_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->review_management_model, $phpfunc))
                    {
                        $data_arr = $this->review_management_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->review_management_model->switchto_fields;
        $extra_cond = $this->review_management_model->extra_cond;
        ...
        $switch_arr = $this->review_management_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->review_management_model->getListConfiguration($alias_name);
    ...
    public function downloadListFile()
    {
        ...
        if (is_array($config_arr) && count($config_arr) > 0)
        {
            ...
            $data_arr = $this->review_management_model->getData(intval($id), $fields, '', '', '', 'Yes');
    ...
    public function uploadFormFile()
    {
        ...
        $config_arr = $this->review_management_model->getFormConfiguration($unique_name);
    ...
    public function deleteFormFile()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $res = $this->review_management_model->update($update_arr, intval($id));
                if (!$res)
                {
