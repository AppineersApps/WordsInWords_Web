...
/**
 * Description of Word Coin Controller
 *
 * @category admin
 *
 * @package tools
 ...
 * @class Word_coin.php
 *
 * @path application\admin\tools\controllers\Word_coin.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Word_coin extends Cit_Controller
{
    /**
     * __construct method is used to set controller preferences while controller object initialization.
     * @created saisita tsavatapalli | 10.01.2020
     * @modified Devangi Nirmal | 27.02.2020
     */
    public function __construct()
    {
        ...
        $this->load->model('word_coin_model');
        $this->folder_name = "tools";
        $this->module_name = "word_coin";
    ...
    public function index()
    {
        ...
        try {
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("word_coin", $access_list, true, true);
            }
            if (!$list_access) {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "word_coin");

            $status_array = array();
            $status_label = array();

            $list_config = $this->word_coin_model->getListConfiguration();
            
            $this->processConfiguration($list_config, $add_access, $edit_access, true);
           
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "word_coin");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->word_coin_model->default_filters,
            );
    ...
    public function listing()
    {
        ...
        $list_config = $this->word_coin_model->getListConfiguration();
        $form_config = $this->word_coin_model->getFormConfiguration();
        $extra_cond = $this->word_coin_model->extra_cond;
        $groupby_cond = $this->word_coin_model->groupby_cond;
        $having_cond = $this->word_coin_model->having_cond;
        $orderby_cond = $this->word_coin_model->orderby_cond;
        ...
        $data_recs = $this->word_coin_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_WORD_COIN_DATA_FOUND_C46_C46_C33');
    ...
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
            $this->filter->checkAccessCapability("word_coin_export");
        } else {
            $this->filter->getModuleWiseAccess("word_coin", "Export", true);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->word_coin_model->getListConfiguration();
        $form_config = $this->word_coin_model->getFormConfiguration();
        $table_name = $this->word_coin_model->table_name;
        $table_alias = $this->word_coin_modeltable_alias;
        $primary_key = $this->word_coin_model->primary_key;
        $extra_cond = $this->word_coin_model->extra_cond;
        $groupby_cond = $this->word_coin_model->groupby_cond;
        $having_cond = $this->word_coin_model->having_cond;
        $orderby_cond = $this->word_coin_model->orderby_cond;
        ...
        $db_recs = $this->word_coin_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('WORD_COIN_WORD_COIN');
        $misc_info['filename'] = "word_coin_records_".count($db_recs);
        ...
        try {
            ...
            if ($mode == "Update") {
                if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("word_coin", $access_list, true, true);
                }
                ...
                if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("word_coin", $access_list, true, true);
                }
                if (!$add_access) {
            ...
            if ($mode == 'Update') {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "word_coin"), $this->session->userdata('iAdminId'));
                $data_arr = $this->word_coin_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "word_coin", $recName);
            } else {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "word_coin"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "word_coin");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->word_coin_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val) {
                    ...
                    if ($val['function'] != "") {
                        ...
                        if (substr($phpfunc, 0, 12) == 'controller::' && substr($phpfunc, 12) !== false) {
                            ...
                            if (method_exists($this->word_coin_model, $phpfunc)) {
                                $tmpdata = $this->word_coin_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                    ...
                    if ($val['field_status'] != "") {
                        ...
                        if ($status_type == "capability" && $fd_callback != "") {
                            ...
                            if (substr($fd_callback, 0, 12) == 'controller::' && substr($fd_callback, 12) !== false) {
                                ...
                                if (method_exists($this->word_coin_model, $fd_callback)) {
                                    $fd_status = $this->word_coin_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
            ...
            if (!empty($render_arr['overwrite_view'])) {
                ...
                if ($mode == "Update") {
                    if ($edit_access && $viewMode != true) {
                        $this->loadView("word_coin_add");
                    } else {
                        $this->loadView("word_coin_add_view");
                    }
                } else {
                    $this->loadView("word_coin_add");
                }
    ...
    public function addAction()
    {
        ...
        try {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                if ($mode == "Update") {
                    $add_edit_access = $this->filter->checkAccessCapability("word_coin_update", true);
                } else {
                    $add_edit_access = $this->filter->checkAccessCapability("word_coin_add", true);
                }
            } else {
                $add_edit_access = $this->filter->getModuleWiseAccess("word_coin", $mode, true, true);
            }
            if (!$add_edit_access) {
                if ($mode == "Update") {
            ...
            $unique_arr = array();
            $unique_arr["wcm_word_length"] = $wcm_word_length;

            $unique_exists = $this->word_coin_model->checkRecordExists($this->word_coin_model->unique_fields, $unique_arr, $id, $mode, $this->word_coin_model->unique_type);
            if ($unique_exists) {
                $error_msg = $this->general->processMessageLabel('ACTION_RECORD_ALREADY_EXISTS_WITH_THESE_DETAILS_OF_LEVEL_NAME_C46_C46_C33');
                if ($error_msg == "") {
                    $error_msg = "Record already exists with these details of Word Length";
                }
            ...
            if ($mode == 'Add') {
                $id = $this->word_coin_model->insert($data);
                if (intval($id) > 0) {
                    $save_data_arr["iWordCoinId"] = $data["iWordCoinId"] = $id;
                ...
                $track_cond = $this->db->protect("wcm.iWordCoinId")." = ".$this->db->escape($id);
                $switch_combo = $this->word_coin_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "word_coin", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            } elseif ($mode == 'Update') {
                $res = $this->word_coin_model->update($data, intval($id));
                if (intval($res) > 0) {
                    $save_data_arr["iWordCoinId"] = $data["iWordCoinId"] = $id;
                ...
                $switch_combo = $this->word_coin_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "word_coin", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status"))) {
            ...
            $config_arr['list_config'] = $this->word_coin_model->getListConfiguration();
            $config_arr['form_config'] = $this->word_coin_model->getFormConfiguration();
            $config_arr['table_name'] = $this->word_coin_model->table_name;
            $config_arr['table_alias'] = $this->word_coin_model->table_alias;
        ...
        if ($search_alias == "Yes") {
            $primary_field = $this->word_coin_model->table_alias.".".$this->word_coin_model->primary_key;
        } else {
            $primary_field = $this->word_coin_model->primary_key;
        }
        ...
        try {
            switch ($operartor) {
                case 'del':
                    $mode = "Delete";
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                        $del_access = $this->filter->checkAccessCapability("word_coin_delete", true);
                    } else {
                        $del_access = $this->filter->getModuleWiseAccess("word_coin", "Delete", true, true);
                    }
                    if (!$del_access) {
                    ...
                    $success = $this->word_coin_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success) {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    $mode = "Update";
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                        $edit_access = $this->filter->checkAccessCapability("word_coin_update", true);
                    } else {
                        $edit_access = $this->filter->getModuleWiseAccess("word_coin", "Update", true, true);
                    }
                    if (!$edit_access) {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    }
                    ...
                    $list_config = $this->word_coin_model->getListConfiguration($post_name);
                    $form_config = $this->word_coin_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0) {
                    ...
                    if (in_array($field_name, $this->word_coin_model->unique_fields)) {
                        $unique_exists = $this->word_coin_model->checkRecordExists($this->word_coin_model->unique_fields, $unique_arr, $primary_ids, "Update", $this->word_coin_model->unique_type);
                        if ($unique_exists) {
                            $error_msg = $this->general->processMessageLabel('ACTION_RECORD_ALREADY_EXISTS_WITH_THESE_DETAILS_OF_LEVEL_NAME_C46_C46_C33');
                            if ($error_msg == "") {
                                $error_msg = "Record already exists with these details of Word Length";
                    ...
                    $success = $this->word_coin_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    if ($search_mode == "Yes" || $search_alias == "Yes") {
                        $field_name = $this->word_coin_model->table_alias.".eStatus";
                    } else {
                        $field_name = $status_field;
                    }
                    $data_arr[$field_name] = $params_arr['status'];
                    $success = $this->word_coin_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success) {
        ...
        switch ($type) {
            ...
            case 'table':
                ...
                if ($combo_config['parent_child'] == "Yes" && $combo_config['nlevel_child'] == "Yes") {
                    $combo_config['main_table'] = $this->word_coin_model->table_name;
            ...
            case 'phpfn':
                ...
                if (substr($phpfunc, 0, 12) == 'controller::' && substr($phpfunc, 12) !== false) {
                    ...
                    $phpfunc = substr($phpfunc, 7);
                    if (method_exists($this->word_coin_model, $phpfunc)) {
                        $data_arr = $this->word_coin_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                } elseif (method_exists($this->general, $phpfunc)) {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->word_coin_model->switchto_fields;
        $extra_cond = $this->word_coin_model->extra_cond;
        ...
        $switch_arr = $this->word_coin_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->word_coin_model->getListConfiguration($alias_name);
