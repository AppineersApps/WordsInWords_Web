...
function validatorHandleMatch($matches){
...
class Validator {
    ...
    public function element($sField, $mValue) {
        ...
        if (isset($this->asClassRule[$sField])) {
            ...
            foreach ($this->asClassRule[$sField] as $sRule => $asRuleParam) {
                if ($sRule == "regex" && is_array($asRuleParam)) {
                    foreach ((array) $asRuleParam as $regKey => $regVal) {
                        if (false === (    # iff empty AND optional: don't check
                                empty($mValue) && # easier checked than next line
                                $this->optional($sField)
                                ) &&
                                false === $this->oMethodCollection->{$sRule}($mValue, $regVal, $this->asValue)) {
                            $this->asErrorMessage[$sField] = $this->getMessageFromArray($sField, $sRule, $asRuleParam, $regKey);
    ...
    public function getMessage($sField, $sRule, $asRuleParam) {
        ...
        return preg_replace('~{(\d+)}~', 'validatorHandleMatch', $sMessage);
    }

    public function getMessageFromArray($sField, $sRule, $asRuleParam, $index) {

        $sMessage = isset($this->asMessage[$sField]) &&
                is_array($this->asMessage[$sField]) &&
                isset($this->asMessage[$sField][$sRule][$index]) ?
                $this->asMessage[$sField][$sRule][$index] : (
                isset($this->asMessage[$sField]) ?
                        $this->asMessage[$sField] :
                        $this->asMessage[$sRule]);
        return preg_replace('~{(\d+)}~', 'validatorHandleMatch', $sMessage);
...
class ValidatorMethodCollection extends ArrayObject {
  ...
  (
     ...
     */
    public function date($sValue, $sSeparator = '/') {
        $currentSec = strtotime($sValue);
        $actualSec = strtotime(date("Y-m-d", strtotime($sValue)));
        return ($currentSec == $actualSec && $currentSec !== false) ? true : false;
    }

    public function datetime($sValue, $sSeparator = '/') {
        $currentSec = strtotime($sValue);
        $actualSec = strtotime(date("Y-m-d H:i:s", strtotime($sValue)));
        return ($currentSec == $actualSec && $currentSec !== false) ? true : false;
    }

    public function time($sValue, $sSeparator = '/') {
        $currentSec = strtotime($sValue);
    ...
    public function required($sValue, $mParam = null, $asFullList = array()) {
        ...
        if(is_array($sValue)){ # array parameter required function 
            return
                false === empty($sValue);
        } else {
