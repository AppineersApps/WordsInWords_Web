...
/**
 * Description of User store review Controller
 *
 * @category admin
 *
 ...
 *
 * @module  User store review
 *
 * @class user_store_review.php
 *
 * @path application\admin\basic_appineers_master\controllers\user_store_review.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class User_store_review extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('user_store_review_model');
        $this->folder_name = "basic_appineers_master";
        $this->module_name = "user_store_review";
        ...
        $this->dropdown_arr = array(
            ...
            "usr_status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('USER_STORE_REVIEW_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('USER_STORE_REVIEW_INACTIVE')
                    )
                )
            )
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "user_store_review_list",
                    "user_store_review_view",
                    "user_store_review_add",
                    "user_store_review_update",
                    "user_store_review_delete",
                    "user_store_review_export",
                    "user_store_review_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("user_store_review", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "user_store_review");
            ...
            $status_label = array(
                'js_lang_label.USER_STORE_REVIEW_PENDING',
                'js_lang_label.USER_STORE_REVIEW_RESOLVED',
                'js_lang_label.USER_STORE_REVIEW_REJECTED',
            );

            $list_config = $this->user_store_review_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "user_store_review");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->user_store_review_model->default_filters,
            );
            ...
            {
                $this->loadView("user_store_review_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->user_store_review_model->getListConfiguration();
        $form_config = $this->user_store_review_model->getFormConfiguration();
        $extra_cond = $this->user_store_review_model->extra_cond;
        $groupby_cond = $this->user_store_review_model->groupby_cond;
        $having_cond = $this->user_store_review_model->having_cond;
        $orderby_cond = $this->user_store_review_model->orderby_cond;
        ...
        $data_recs = $this->user_store_review_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_USER_STORE_REVIEW_DATA_FOUND_C46_C46_C33');

        $data_recs = $this->user_store_review_model->processCustomLinks($data_recs, $list_config);
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("user_store_review_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("user_store_review", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->user_store_review_model->getListConfiguration();
        $form_config = $this->user_store_review_model->getFormConfiguration();
        $table_name = $this->user_store_review_model->table_name;
        $table_alias = $this->user_store_review_modeltable_alias;
        $primary_key = $this->user_store_review_model->primary_key;
        $extra_cond = $this->user_store_review_model->extra_cond;
        $groupby_cond = $this->user_store_review_model->groupby_cond;
        $having_cond = $this->user_store_review_model->having_cond;
        $orderby_cond = $this->user_store_review_model->orderby_cond;
        ...
        $db_recs = $this->user_store_review_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('USER_STORE_REVIEW_USER_STORE_REVIEW');
        $misc_info['filename'] = "user_store_review_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("user_store_review_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("user_store_review", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("user_store_review", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "user_store_review"), $this->session->userdata('iAdminId'));
                $data_arr = $this->user_store_review_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "user_store_review", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "user_store_review"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "user_store_review");
            }
            ...
            if ($mode == "Update")
            {
                ...
                $main_data = $this->user_store_review_model->getData(intval($id), array('usr.iUserReviewId'));
            }
            else
            {
                $main_data = array();
            }
            $relation_module = $this->user_store_review_model->relation_modules["user_review_images"];
            ...
            if ($mode == "Update")
            {
                $extra_cond = $this->db->protect("usr.iUserReviewId")." = ".$this->db->escape($main_data[0]["iUserReviewId"]);
            ...
            $form_config = $this->user_store_review_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->user_store_review_model, $phpfunc))
                            {
                                $tmpdata = $this->user_store_review_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->user_store_review_model, $fd_callback))
                                {
                                    $fd_status = $this->user_store_review_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("user_store_review_add");
                    }
                    else
                    {
                        $this->loadView("user_store_review_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("user_store_review_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("user_store_review_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("user_store_review", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $usr_user_id = $params_arr["usr_user_id"];
            $usr_comment = $params_arr["usr_comment"];
            $usr_added_at = $params_arr["usr_added_at"];
           
            $usr_status = $params_arr["usr_status"];
            $usr_updated_at = $params_arr["usr_updated_at"];

            $data = $save_data_arr = $file_data = array();
            $data["iUserId"] = $usr_user_id;
            $data["tComment"] = $usr_comment;
            $data["dtAddedAt"] = $this->filter->formatActionData($usr_added_at, $form_config["usr_added_at"]);
           
            $data["eStatus"] = $usr_status;
            $data["dtUpdatedAt"] = $this->filter->formatActionData($usr_updated_at, $form_config["usr_updated_at"]);

            $save_data_arr["usr_user_id"] = $data["iUserId"];
            $save_data_arr["usr_comment"] = $data["tComment"];
            $save_data_arr["usr_added_at"] = $data["dtAddedAt"];
          
            $save_data_arr["usr_status"] = $data["eStatus"];
            $save_data_arr["usr_updated_at"] = $data["dtUpdatedAt"];
            if ($mode == 'Add')
            {
                $id = $this->user_store_review_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iUserReviewId"] = $data["iUserReviewId"] = $id;
                ...
                $track_cond = $this->db->protect("usr.iUserReviewId")." = ".$this->db->escape($id);
                $switch_combo = $this->user_store_review_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "user_store_review", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->user_store_review_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iUserReviewId"] = $data["iUserReviewId"] = $id;
                ...
                $switch_combo = $this->user_store_review_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "user_store_review", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
                ...
                foreach ((array) $childModuleArr as $mdKey => $mdVal)
                {
                    ...
                    switch ($child_module)
                    {
                        case "user_review_images":
                            ...
                            $extra_del = $this->db->protect("uri.iUserReviewId")." = ".$this->db->escape($main_data[0]["iUserReviewId"])." AND ".$this->db->protect("uri.iUserReviewImageId")." NOT IN ('".implode("','", $child_del_ids)."')";
                            if ($relation_module["extra_cond"] != "")
                            {
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->user_store_review_model->getListConfiguration();
            $config_arr['form_config'] = $this->user_store_review_model->getFormConfiguration();
            $config_arr['table_name'] = $this->user_store_review_model->table_name;
            $config_arr['table_alias'] = $this->user_store_review_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->user_store_review_model->table_alias.".".$this->user_store_review_model->primary_key;
        }
        else
        {
            $primary_field = $this->user_store_review_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("user_store_review_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("user_store_review", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $success = $this->user_store_review_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("user_store_review_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("user_store_review", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->user_store_review_model->getListConfiguration($post_name);
                    $form_config = $this->user_store_review_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $success = $this->user_store_review_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->user_store_review_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    $success = $this->user_store_review_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->user_store_review_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->user_store_review_model, $phpfunc))
                    {
                        $data_arr = $this->user_store_review_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->user_store_review_model->switchto_fields;
        $extra_cond = $this->user_store_review_model->extra_cond;
        ...
        $switch_arr = $this->user_store_review_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->user_store_review_model->getListConfiguration($alias_name);
        ...
        try
        {
            ...
            switch ($child_module)
            {
                case "user_review_images":
                    ...
                    if ($recMode == "Update")
                    {
                        $child_mod_data = $this->user_review_images_model->getData(intval($id), "", "", "", "", "Yes");
                    ...
                    $child_module_add_file = "user_store_review_user_review_images_add_more";
                    break;
            }
        ...
        try
        {
            ...
            switch ($child_module)
            {
                case "user_review_images":
                    ...
                    $data_arr = $this->user_review_images_model->getData(intval($id));
                    ...
                    $usri_user_query_id = $child_post["usr_user_review_id"][$index];
                    $usri_query_image = $child_post["usr_review_image"][$index];
                    $usri_added_at = $child_post["usr_added_at"][$index];
                    $usri_status = $child_post["usr_status"][$index];

                    $child_data["iUserReviewId"] = $usr_user_review_id;
                    $child_data["vReviewImage"] = $usri_review_image;
                    $child_data["dtAddedAt"] = $this->filter->formatActionData($usri_added_at, $form_config["usri_added_at"]);
                    $child_data["eStatus"] = $usri_status;

                    $child_data["iUserReviewId"] = $params_arr["parID"];
                    if ($mode == "Add")
                    {
                        $id = $res = $this->usri_review_images_model->insert($child_data);
                        if (intval($id) > 0)
                        {
                            $data["iUserReviewImageId"] = $id;
                            $save_data_arr["iUserReviewImageId"] = $child_data["iUserReviewImageId"] = $id;
                            $msg = $this->general->processMessageLabel('ACTION_RECORD_ADDED_SUCCESSFULLY_C46_C46_C33');
                        }
                        else
                        {
                    ...
                    elseif ($mode == "Update")
                    {
                        $res = $this->user_review_images_model->update($child_data, intval($id));
                        if (intval($res) > 0)
                        {
                            $data["iUserReviewImageId"] = $id;
                            $save_data_arr["iUserReviewImageId"] = $child_data["iUserReviewImageId"] = $id;
                    ...
                    $file_data["usr_store_image"]["file_name"] = $usri_query_image;
                    $file_data["usr_store_image"]["old_file_name"] = $child_post["old_usr_store_image"];
                    $file_data["usr_store_image"]["unique_name"] = "usr_store_image";
                    $file_data["usr_store_image"]["primary_key"] = "iUserReviewImageId";
                    $file_keep = $form_config["usr_store_image"]["file_keep"];
                    if ($file_keep != "" && $file_keep != "iUserReviewImageId")
                    {
    ...
    {
        $config_arr["child_module"]= 'user_review_images';
        ...
        try
        {
            ...
            switch ($child_module)
            {
                case "user_review_images":
                    ...
                    $res = $this->user_review_images_model->delete($where_cond, "Yes", $parent_join_arr);
                    if (!$res)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
