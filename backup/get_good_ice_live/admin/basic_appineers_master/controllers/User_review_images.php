...
/**
 * Description of review Images Controller
 *
 * @category admin
 *
 ...
 *
 * @class user_review_images.php
 *
 * @path application\admin\basic_appineers_master\controllers\user_review_images.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class User_review_images extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('user_review_images_model');
        $this->folder_name = "basic_appineers_master";
        $this->module_name = "user_review_images";
        ...
        $this->dropdown_arr = array(
            ...
            "uri_status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('USER_REVIEW_IMAGES_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('USER_REVIEW_IMAGES_INACTIVE')
                    )
                )
            )
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "user_review_images_list",
                    "user_review_images_view",
                    "user_review_images_add",
                    "user_review_images_update",
                    "user_review_images_delete",
                    "user_review_images_export",
                    "user_review_images_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("user_review_images", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "user_review_images");
            ...
            $status_label = array(
                'js_lang_label.USER_REVIEW_IMAGES_ACTIVE',
                'js_lang_label.USER_REVIEW_IMAGES_INACTIVE',
            );

            $list_config = $this->user_review_images_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "user_review_images");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->user_review_images_model->default_filters,
            );
            ...
            {
                $this->loadView("user_review_images_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->user_review_images_model->getListConfiguration();
        $form_config = $this->user_review_images_model->getFormConfiguration();
        $extra_cond = $this->user_review_images_model->extra_cond;
        $groupby_cond = $this->user_review_images_model->groupby_cond;
        $having_cond = $this->user_review_images_model->having_cond;
        $orderby_cond = $this->user_review_images_model->orderby_cond;
        ...
        $data_recs = $this->user_review_images_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_user_review_images_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("user_review_images_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("user_review_images", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->user_review_images_model->getListConfiguration();
        $form_config = $this->user_review_images_model->getFormConfiguration();
        $table_name = $this->user_review_images_model->table_name;
        $table_alias = $this->user_review_images_modeltable_alias;
        $primary_key = $this->user_review_images_model->primary_key;
        $extra_cond = $this->user_review_images_model->extra_cond;
        $groupby_cond = $this->user_review_images_model->groupby_cond;
        $having_cond = $this->user_review_images_model->having_cond;
        $orderby_cond = $this->user_review_images_model->orderby_cond;
        ...
        $db_recs = $this->user_review_images_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('USER_REVIEW_IMAGES_USER_REVIEW_IMAGES');
        $misc_info['filename'] = "user_review_images_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("user_review_images_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("user_review_images", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("user_review_images", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "user_review_images"), $this->session->userdata('iAdminId'));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "user_review_images", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "user_review_images"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "user_review_images");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->user_review_images_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->user_review_images_model, $phpfunc))
                            {
                                $tmpdata = $this->user_review_images_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->user_review_images_model, $fd_callback))
                                {
                                    $fd_status = $this->user_review_images_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("user_review_images_add");
                    }
                    else
                    {
                        $this->loadView("user_review_images_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("user_review_images_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("user_review_images_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("user_review_images", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $uri_user_review_id = $params_arr["uri_user_review_id"];
            $uri_user_review_image = $params_arr["uri_user_review_image"];
            $uri_added_at = $params_arr["uri_added_at"];
            $uri_status = $params_arr["uri_status"];

            $data = $save_data_arr = $file_data = array();
            $data["iUserReviewId"] = $uqi_user_review_id;
            $data["vReviewImage"] = $uqi_review_image;
            $data["dtAddedAt"] = $this->filter->formatActionData($uqi_added_at, $form_config["uri_added_at"]);
            $data["eStatus"] = $uqi_status;

            $save_data_arr["uri_user_review_id"] = $data["iUserReviewId"];
            $save_data_arr["uri_review_image"] = $data["vReviewImage"];
            $save_data_arr["uri_added_at"] = $data["dtAddedAt"];
            $save_data_arr["uri_status"] = $data["eStatus"];
            if ($mode == 'Add')
            {
                $id = $this->user_review_images_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iUserReviewImageId"] = $data["iUserReviewImageId"] = $id;
                ...
                $track_cond = $this->db->protect("uri.iUserReviewImageId")." = ".$this->db->escape($id);
                $switch_combo = $this->user_review_images_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "user_review_images", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->user_review_images_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iUserReviewImageId"] = $data["iUserReviewImageId"] = $id;
                ...
                $switch_combo = $this->user_review_images_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "user_review_images", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
            ...
            $file_data["uri_user_store_image"]["file_name"] = $uri_user_store_image;
            $file_data["uri_user_store_image"]["old_file_name"] = $params_arr["old_uri_user_store_image"];
            $file_data["uri_user_store_image"]["unique_name"] = "uri_review_image";
            $file_data["uri_user_store_image"]["primary_key"] = "iUserReviewImageId";
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->user_review_images_model->getListConfiguration();
            $config_arr['form_config'] = $this->user_review_images_model->getFormConfiguration();
            $config_arr['table_name'] = $this->user_review_images_model->table_name;
            $config_arr['table_alias'] = $this->user_review_images_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->user_review_images_model->table_alias.".".$this->user_review_images_model->primary_key;
        }
        else
        {
            $primary_field = $this->user_review_images_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("user_review_images_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("user_review_images", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    if (is_array($primary_ids))
                    {
                        $main_cond = $this->db->protect("uri.iUserReviewImageId")." IN ('".implode("','", $primary_ids)."')";
                    }
                    else
                    {
                        $main_cond = $this->db->protect("uri.iUserReviewImageId")." = ".$this->db->escape($primary_ids);
                    }
                    $main_data = $this->user_review_images_model->getData($main_cond, array('uri.vReviewImage', 'uri.iUserReviewId'));
                    $success = $this->user_review_images_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("user_review_images_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("user_review_images", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->user_review_images_model->getListConfiguration($post_name);
                    $form_config = $this->user_review_images_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $db_rec_arr = $this->user_review_images_model->getData(intval($primary_ids));

                    $data_arr[$field_name] = $post_val;
                    $success = $this->user_review_images_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->user_review_images_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    $success = $this->user_review_images_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
            ...
            if ($mode == "Update")
            {
                ...
                if ($form_config["type"] == "file")
                {
                    ...
                    $file_data[$unique_name]["primary_key"] = "iUserReviewImageId";
     ...
     * @param string $extra extra review condition for searching data array.
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->user_review_images_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->user_review_images_model, $phpfunc))
                    {
                        $data_arr = $this->user_review_images_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->user_review_images_model->switchto_fields;
        $extra_cond = $this->user_review_images_model->extra_cond;
        ...
        $switch_arr = $this->user_review_images_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->user_review_images_model->getListConfiguration($alias_name);
    ...
    public function uploadFormFile()
    {
        ...
        $config_arr = $this->user_review_images_model->getFormConfiguration($unique_name);
    ...
    public function downloadFormFile()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $data_arr = $this->user_review_images_model->getData(intval($id), $fields, '', '', '', 'Yes');
    ...
    public function deleteFormFile()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $res = $this->user_review_images_model->update($update_arr, intval($id));
                if (!$res)
                {
    ...
    public function getRelationModule($config_arr = array())
    {
        ...
        if ($callback_func != "")
        {
            $return_arr['status'] = $this->filter->processModuleFunction($callback_func, $this, $this->user_review_images_model, $config_arr);
        }
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $edit_access = $this->filter->checkAccessCapability("user_review_images_update", TRUE);
        ...
        if (in_array('data', $perform_arr) || in_array('options', $perform_arr))
        {
            ...
            foreach ($data_arr as $dKey => $dVal)
            {
                ...
                foreach ($form_config as $key => $val)
                {
                    $source_field = $val['name'];
                    $data = $dVal[$key];
                    $id = $dVal[$this->user_review_images_model->primary_key];
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->user_review_images_model, $phpfunc))
                            {
                                $tmpdata = $this->user_review_images_model->$phpfunc($mode, $data, $dVal, $id, $source_field, $combo_id, $combo_name, $config_arr);
                            }
                        }
                        elseif (method_exists($this->general, $phpfunc))
                        {
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->user_review_images_model, $fd_callback))
                                {
                                    $fd_status = $this->user_review_images_model->$fd_callback($mode, $data, $dVal, $id, $source_field, $combo_id, $combo_name, $config_arr);
                                }
                            }
                            elseif (method_exists($this->general, $fd_callback))
                            {
    ...
    public function addActionPopup()
    {
        ...
        try
        {
            ...
            $form_config = $this->user_review_images_model->getFormConfiguration();
            $uqi_user_review_id = $params_arr["uqi_user_review_id"];
            $uqi_review_image = $params_arr["uqi_review_image"];
            ...
            $save_data_arr["uqi_user_review_id"] = $data["iUserReviewId"];
            $save_data_arr["uqi_review_image"] = $data["vReviewImage"];
            $save_data_arr["uqi_added_at"] = $data["dtAddedAt"];
            $save_data_arr["uqi_status"] = $data["eStatus"];
            if ($mode == 'Update')
            {
                $save_data_arr[$this->user_review_images_model->primary_key] = $id;
                $save_data_arr[$this->user_review_images_model->primary_alias] = $id;
            }
