    ...
    <div class="<%$module_name%>" data-form-name="<%$module_name%>">
        <div id="ajax_content_div" class="ajax-content-div top-frm-spacing" >
            ...
            <div id="scrollable_content" class="scrollable-content popup-content top-block-spacing ">
                <div id="user_store_review" class="frm-module-block frm-elem-block frm-stand-view">
                    <!-- Module Form Block -->
                                    ...
                                    <div class="content <%$rl_theme_arr['frm_stand_label_align']%>">
                                        <div class="form-row row-fluid " id="cc_sh_usr_user_id">
                                            ...
                                            <div class="form-right-div  <%if $mode eq 'Update'%>frm-elements-div<%/if%> ">
                                                <%assign var="opt_selected" value=$data['usr_user_id']%>
                                                <%if $mode eq "Update"%>
                                                    <input type="hidden" name="usr_user_id" id="usr_user_id" value="<%$data['usr_user_id']%>" class="ignore-valid"/>
                                                    <%assign var="combo_arr" value=$opt_arr["usr_user_id"]%>
                                                    ...
                                                    <%else%>
                                                        <%$this->dropdown->display("usr_user_id","usr_user_id","  title='<%$this->lang->line('USER_STORE_REVIEW_FULL_NAME')%>'  aria-chosen-valid='Yes'  class='chosen-select frm-size-medium'  data-placeholder='<%$this->general->parseLabelMessage('GENERIC_PLEASE_SELECT__C35FIELD_C35' ,'#FIELD#', 'USER_STORE_REVIEW_FULL_NAME')%>'  ", "|||", "", $opt_selected,"usr_user_id")%>
                                                    <%/if%>
                                                </div>
                                                <div class="error-msg-form "><label class='error' id='usr_user_idErr'></label></div>
                                            </div>
                                            <div class="form-row row-fluid " id="cc_sh_usr_feedback">
                                                ...
                                                <div class="form-right-div  <%if $mode eq 'Update'%>frm-elements-div<%/if%> ">
                                                    <%if $mode eq "Update"%>
                                                        <textarea style="display:none;" class="ignore-valid" name="usr_comment" id="usr_comment"><%$data['usr_comment']%></textarea>
                                                        <span class="frm-data-label">
                                                            <strong>
                                                                <%if $data['usr_comment'] neq ""%>
                                                                    <%$data['usr_comment']%>
                                                                <%else%>
                                                                <%/if%>
                                                            </strong></span>
                                                        <%else%>
                                                            <textarea placeholder=""  name="usr_comment" id="usr_comment" title="<%$this->lang->line('USER_STORE_REVIEW_USER_STORE_REVIEW')%>"  data-ctrl-type='textarea'  class='elastic frm-size-medium'  ><%$data['usr_comment']%></textarea>
                                                        <%/if%>
                                                    </div>
                                                    <div class="error-msg-form "><label class='error' id='usr_commentErr'></label></div>
                                                </div>
                                                <div class="form-row row-fluid " id="cc_sh_usr_added_at">
                                                    ...
                                                    <div class="form-right-div  <%if $mode eq 'Update'%>frm-elements-div<%else%>input-append text-append-prepend<%/if%> ">
                                                        <%if $mode eq "Update"%>
                                                            <input type="hidden" name="usr_added_at" id="usr_added_at" value="<%$this->general->dateSystemFormat($data['usr_added_at'])%>" class="ignore-valid view-label-only"  data-ctrl-type='date'  class='frm-datepicker ctrl-append-prepend frm-size-medium'  aria-date-format='<%$this->general->getAdminJSFormats('date', 'dateFormat')%>'  aria-format-type='date' />
                                                            <%assign var="display_date" value=$this->general->dateSystemFormat($data['usr_added_at'])%>
                                                            <span class="frm-data-label">
                                                                <strong>
                                                                    <%if $display_date neq ""%>
                                                                        <%$display_date%>
                                                            ...
                                                            <%else%>
                                                                <input type="text" value="<%$this->general->dateSystemFormat($data['usr_added_at'])%>" placeholder="" name="usr_added_at" id="usr_added_at" title="<%$this->lang->line('USER_STORE_REVIEW_REPORTED_ON')%>"  data-ctrl-type='date'  class='frm-datepicker ctrl-append-prepend frm-size-medium'  aria-date-format='<%$this->general->getAdminJSFormats('date', 'dateFormat')%>'  aria-format-type='date'  />
                                                                <span class='add-on text-addon date-append-class icomoon-icon-calendar'></span>
                                                            <%/if%>
                                                        </div>
                                                        <div class="error-msg-form "><label class='error' id='usr_added_atErr'></label></div>
                                                    </div>
                                                    
                                                                <%assign var="child_module_name" value="user_review_images"%>
                                                                <%if $child_assoc_status[$child_module_name] eq 1%>
                                                                    <%assign var="child_module_name" value="user_store_images"%>
                                                                    ...
                                                                    <div class="form-row row-fluid form-inline-child" id="child_module_<%$child_module_name%>">
                                                                        ...
                                                                        <input type="hidden" name="childModuleSingle[<%$child_module_name%>]" id="childModuleSingle_<%$child_module_name%>" value="uri_review_image"/>
                                                                        <input type="hidden" name="childModuleFileDesc[<%$child_module_name%>]" id="childModuleFileDesc_<%$child_module_name%>" value=""/>
                                                                        <input type="hidden" name="childModuleParField[<%$child_module_name%>]" id="childModuleParField_<%$child_module_name%>" value="iUserReviewId"/>
                                                                        <input type="hidden" name="childModuleParData[<%$child_module_name%>]" id="childModuleParData_<%$child_module_name%>" value="<%$this->general->getAdminEncodeURL($data['iUserReviewId'])%>"/>
                                                                        ...
                                                                        <div class="form-right-div form-inline-child <%if $recMode eq "Update"%>frm-elements-div<%/if%>"  id="child_module_rel_<%$child_module_name%>">
                                                                            <%assign var="cb_uri_review_image" value=$child_func[0]["uri_review_image"]%>
                                                                            <%if $recMode eq "Update" && $mode eq "Update"%>
                                                                                <!-- View Only -->
                                                                            <%else%>
                                                                                <div  class='btn-uploadify frm-size-medium' >
                                                                                    <div id="upload_drop_zone_child_user_review_images_uri_review_image_0" class="upload-drop-zone"></div>
                                                                                    <div class="uploader upload-src-zone">
                                                                                        <input type="file" name="uploadify_child[user_review_images][uri_review_image][0]" id="uploadify_child_user_review_images_uri_review_image_0" title="<%$child_conf_arr['form_config']['uri_review_image']['label_lang']%>" multiple=true />
                                                                                        <span class="filename" id="preview_child_user_review_images_uri_review_image_0"><%$this->lang->line('GENERIC_DROP_FILES_HERE_OR_CLICK_TO_UPLOAD')%></span>
                                                                                        <span class="action">Choose File</span>
                                                                                    </div>
                                                                                </div> 
                                                                            <%/if%>
                                                                            <span class="file-viewer">
                                                                                <a href="javascript://" class="viewer-anchor tipR" style="text-decoration:none;" data-viewer-target="upload_multi_file_user_review_images" data-viewer-loop="row-upload-file" data-viewer-ext="gif,png,jpg,jpeg,jpe,bmp,ico">
                                                                            ...
                                                                            <div class='clear upload-progress' id='progress_child_user_review_images_uri_review_image_0' >
                                                                                <div class='upload-progress-bar progress progress-striped active'>
                                                                                    <div class='bar' id='practive_child_user_review_images_uri_review_image_0'></div>
                                                                                </div>
                                                                        ...
                                                                        <div class="error-msg-form inline-module-error">
                                                                            <label class='error' id='child_user_review_images_uri_review_image_0Err'></label>
                                                                        </div>
                                                                    </div>
                                                                ...
                                                                <!-- <div class="form-row row-fluid " id="cc_sh_usr_note">
                                                                    ...
                                                                    <div class="form-right-div  ">
                                                                        <textarea placeholder=""  name="usr_note" id="usr_note" title="<%$this->lang->line('USER_STORE_REVIEW_NOTES')%>"  data-ctrl-type='textarea'  class='elastic frm-size-medium'  ><%$data['usr_note']%></textarea>
                                                                    </div>
                                                                    <div class="error-msg-form "><label class='error' id='usr_noteErr'></label></div>
                                                                </div> -->
                                                                <div class="form-row row-fluid " id="cc_sh_usr_status">
                                                                    ...
                                                                    <div class="form-right-div  ">
                                                                        <%assign var="opt_selected" value=$data['usr_status']%>
                                                                        <%$this->dropdown->display("usr_status","usr_status","  title='<%$this->lang->line('USER_STORE_REVIEW_STATUS')%>'  aria-chosen-valid='Yes'  class='chosen-select frm-size-medium'  data-placeholder='<%$this->general->parseLabelMessage('GENERIC_PLEASE_SELECT__C35FIELD_C35' ,'#FIELD#', 'USER_STORE_REVIEW_STATUS')%>'  ", "|||", "", $opt_selected,"usr_status")%>
                                                                    </div>
                                                                    <div class="error-msg-form "><label class='error' id='usr_statusErr'></label></div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="frm-bot-btn <%$rl_theme_arr['frm_stand_action_bar']%> <%$rl_theme_arr['frm_stand_action_btn']%> popup-footer">
                                                        ...
                                                        <%/if%>
                                                        <%include file="user_store_review_add_buttons.tpl"%>
                                                    </div>
                                                </div>
                                                <div class="clear"></div>
                                            </form>
...
<%javascript%>
    ...
    child_rules_arr = {
        "user_review_images": {
            "uri_user_review_id": {
                "type": "dropdown",
                "vUniqueName": "uri_user_review_id",
                "editrules": {
                    "infoArr": []
                }
            },
    ...
    el_form_settings['message_arr'] = {
        ...
        "child_messages" : {
            "delete_message" : {
        "user_review_images" : "<%$this->general->processMessageLabel('ACTION_ARE_YOU_SURE_WANT_TO_DELETE_THIS_RECORD_C63')%>",
            }
        }
    };
