...
class Set_favorite_icetype_model extends CI_Model
{
     ...
     */
    public function Set_favorite_icetype($params_arr = array())
    {
        try
        {
            $result_arr = array();
            ...
            if (isset($params_arr["icetype_id"]))
            {
                $this->db->set("iicetypeId", $params_arr["icetype_id"]);
            }
            ...
            $this->db->insert("favorite_ice_type");
     ...
     * @return array $return_arr returns response of review block.
     */
    public function get_favorite_store_details($user_id = '',$icetype_id='')
    {
        try
        {
            $result_arr = array();
            ...
            $this->db->select("fit.iId AS fit_Id");
            if (isset($user_id) && $user_id != "")
            {
                $this->db->where("fit.iUserId =", $user_id);
            }
            if (isset($icetype_id) && $icetype_id != "")
            {
                $this->db->where("fit.iIceTypeId =", $icetype_id);
            }
     ...
     */
    public function update_favorite_icetype($params_arr = array(), $where_arr = array())
    {
        try
        {
            $result_arr = array();
            ...
            if (isset($where_arr["icetype_id"]) && $where_arr["icetype_id"] != "")
            {
                $this->db->where("iIceTypeId =", $where_arr["icetype_id"]);
            }
            if (isset($params_arr["status"]))
            {
                $this->db->set("eStatus", $params_arr["status"]);
            ...
            $this->db->set($this->db->protect("dtUpdatedAt"), $params_arr["_dtupdatedat"], FALSE);
            $res = $this->db->update("favorite_ice_type");
